...
class FaissHNSW:
    ...
    def __init__(self, dim, **kwargs):
        ...
        m, efConstruction = kwargs.get("M", 32), kwargs.get("efConstruction", 40) #  # Default M is 32; Default efConstruction is 40
        self.metric = kwargs.get("metric", None)

        if not self.metric:
            # Create HNSW index with default settings
            self.index = faiss.IndexHNSWFlat(dim, m)

        elif self.metric.lower() in ['euclidean', 'minkowski']:
            # Create HNSW index with Euclidean (L2) distance
            self.index = faiss.IndexHNSWFlat(dim, m, faiss.METRIC_L2)
        
        elif self.metric.lower() in ['cosine', 'dot_product']:
            # Create HNSW index with Inner Product (equivalent to cosine similarity after normalization)
            self.index = faiss.IndexHNSWFlat(dim, m, faiss.METRIC_INNER_PRODUCT)

        else:
            raise ValueError("Unsupported metric. Use 'euclidean', 'minkowski', 'cosine', 'dot_product'.")
